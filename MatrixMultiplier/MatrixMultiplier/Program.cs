...
namespace MatrixMultiplier
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            ILog calculationLog = new ThreadSafeLog();

            var watch = Stopwatch.StartNew();
            var result = new ThreadedMatrixMultiplier(threadCount, calculationLog).MultiplyMatrices(matrixSize, matrix1, matrix2);
            watch.Stop();
            ...
            using (var writer = new StreamWriter(File.Create("log.txt")))
            {
                foreach (var logMessage in calculationLog.GetLogContent())
                {
